...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Unit kerja </h5>
			<hr />
...
{
	$tambahan .= nbs(2)."<a href='".site_url('setting/tambah-unit')."' class='btn btn-sm btn-default' id='TambahUnit'><i class='fa fa-plus fa-fw'></i> Tambah Unit Kerja</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('setting/unit-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#HapusUnit', function(e){
		e.preventDefault();
		...
		$('#ModalFooter').html("<button type='button' class='btn btn-danger' id='YesDeleteUnit' data-url='"+Link+"'>Ya, saya yakin</button><button type='button' class='btn btn-default' data-dismiss='modal'>Batal</button>");
		$('#ModalGue').modal('show');
	});

	$(document).on('click', '#YesDeleteUnit', function(e){
		e.preventDefault();
		$('#ModalGue').modal('hide');
	...
	$(document).on('click', '#TambahUnit, #EditUnit', function(e){
		...
		{
			$('#ModalHeader').html('Tambah Unit Kerja');
		}
		if($(this).attr('id') == 'EditUnit')
		{
			$('#ModalHeader').html('Edit Unit Kerja');
		}
