...
echo "</td>
	...
	<tr>
				...
				<td>";
		if($master->acc == 'Belum')
			{echo "Pinjaman Belum di ACC";}else{ echo "Pinjaman Sudah di ACC";}
...
<div class='row'>
								<div class='col-sm-6'><p style="color: red;line-height: 1;font-size: 12px;">* Jika Sudah di ACC maka Tombol untuk Cetak Surat Perintah Membayar Uang dan Tombol untuk Download Surat Perjanjian Pinjaman sudah bisa di Print >>>></p></div>
								<div class='col-sm-3'>
								<button type='button' class='btn btn-default' id='CetakSurat'>
											<i class='fa fa-print'></i> Cetak Surat Perintah
										</button>
								</div>
							<div class='col-sm-3'>
...
<table id="my-grid" class="table tabel-transaksi table-bordered" style='margin-bottom: 0px; margin-top: 10px;'>
	...
	echo "<tr style='background:#deeffc;'>
			<td >"?><div class='form-group'>
			 <small class="text-muted"><i class="fa fa-time"></i>Edit Status Pinjaman  </small>
	<select name='acc' id='acc' class='form-control'>
		
		<option value="ACC" <?php if($master->acc == 'ACC') { echo 'selected'; } ?>>ACC</option>
		<option value="DiTolak" <?php if($master->acc == 'DiTolak') { echo 'selected'; } ?>>TOLAK</option>
	</select>
</div><?php echo "</td>
